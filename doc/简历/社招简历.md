...
你好，我交大毕业后就一直在美团做java后端开发的工作，目前有2年的工作经验。在美团我主要是负责酒旅优惠券业务的研发，也会参与一些大促的酒旅业务的保障工作。在开发中，数据存储的话是用的mysql，以及redis，消息队列的话我们公司用的都是fakfa。此外，由于会有一些高并发地场景，所以也会用到限流，熔断降级，分布式锁等组件。很开心能够参加贵公司的面试。

第一个项目是一个技术需求，之前到店每个业务（比如酒店，到店餐饮等）都会有自己的券系统，但是其实优惠券的很多逻辑是可以复用的，所以可以把不同业务的券的逻辑都放到一个系统中来进行维护。 我这个项目做的就是要将机票业务券系统迁移到我们新的平台券系统中。第一个挑战是如何快速熟悉业务逻辑。第二个挑战是，迁移不是直接把代码复制过去就行了，需要在保证原有功能不变的情况下复用平台系统的逻辑。第三个就是如何保证迁移过来的逻辑是否是正确的。

对于第一个挑战，我这边的做法就是找之前的机票系统的同学要他们系统的源码以及相关的文档进行逻辑梳理，对于一些有疑问的内容和机票业务的研发和产品进行对齐。
...
对于第三个挑战，应对方式除了自测和测试同学的测试外，还搭建一套比对工具，为了避免影响线上用户，比对期间的比对数据都是操作比对库。在比对的时候也遇到一些问题，发券比对需要监听mafka消息，由于消息积压，线上流量已经发券了，但是比对流量还没有发券，这时候查询的时候会出现查到的券不一致的情况， 我这边的做法就是对于消费者通过多线程的方式来进行消费，提高消费能力。还有就是在一些发券流量高峰期，通过扩容的方式来提高消费能力。



第二个项目是一个业务需求，核心诉求就是在C端浏览侧查询用户券时，不仅要返回用户已有可用的优惠券，还要返回用户可以领同时可以用的优惠券。项目主要分为两期。

一期主要就是实现基本的功能，运营配置可领券数量比较少。对于第一期来说，挑战就是浏览侧QPS较高，对耗时要求也较高，而这次需求需要新增一个判断是否可领的RPC调用，对耗时影响比较大。对于这个挑战，我优化新增的发券预校验模块，使用 Future 并行加载券批次剩余库存和每个用户的发券数量，频次等数据信息，减少了 RPC 调用共 70 ms；为了减少对库存服务的压力，在查询校验服务添加了本地缓存，并通过定时任务自动续期。此外，为了提升用户的体验和系统稳定性，还使用了限流，熔断降级的手段。

项目二期需要支持更多可领券的召回，为此，我这边进行了单机压测和火焰图分析，发现影响性能的主要原因是， 在拿到可领券批次ID后，会查询券批次的信息，后续会对券批次的信息进行深拷贝。券数据越多，性能就越差。与运营和产品沟通后，发现每个可领券的批次区分度比较大的是适用的商品。而一期C端是通过某个区分度不大的特征来获取可领券批次的。所以二期采用了通过商品ID查询可领券批次，然后再查询券批次信息的方式进行提前过滤。



第三个项目主要是一个管理端的需求，核心诉求是需要为商家搭建一套管理券，查询券和发券的系统。其中主要解决两个问题，第一个是并发修改的问题，我是采用乐观锁的方式来解决的，第二个是为了满足商家端的一些条件查询，在最上层的服务冗余了一些底层的数据。所以需要保证冗余数据和底层数据的一致性，我的做法是在每次写操作后通过消息队列发送操作的券ID，然后进行实时的对账。
